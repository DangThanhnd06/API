<?php
    ...
    $s = $h->query("select id from practise where clg='".$data['clg']."' and (name_vn='".$data['name_vn']."' or name_en='".$data['name_vn']."')");
    $n = $h->num_rows($s);
    if($n) echo '2';
    else {
        $ss = $h->insert($data,"practise");
        $sc = $h->query("select id,name_vn,name_en from practise where clg='".$data['clg']."' order by id desc");
        $rc = $h->fetch_array($sc);
        $nc = $h->num_rows($sc);
        if($nc%2==0) $cl = ' odd';
        else $cl = ' even';
