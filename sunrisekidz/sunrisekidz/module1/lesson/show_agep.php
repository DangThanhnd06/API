<?php
    ...
    $age = $_POST['age'];
    $yr = date("Y");
    $sp = $h->query("select id,name_vn,name_en from practise where age=$age and hide=1 order by sort asc,id asc");
    $np = $h->num_rows($sp);
    $msg = "";
    $msm = "";
    $msd = "";
    ...
    if($np){
        ...
        while($rc = $h->fetch_array($sp)) {
            ...
            $name = $rc["name_$lang"];
            $msg .= '<div class="cten'.$cl.$ca.'"><a class="cp" rel="'.$idd.'">'.$name.'</a></div>';
            $nc++;
        }
