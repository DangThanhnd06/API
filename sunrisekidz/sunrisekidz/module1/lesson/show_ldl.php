<?php
    ...
    $msm = "";
    $msd = "";
    $sy = $h->query("select age from practise where id=$pr_id");
    $ry = $h->fetch_array($sy);
    $age = $ry['age'];
        ...
        #$idp = $rpp['id'];
        $sm = $h->query("select id,name_vn,name_en from monhoc where pr_id=".$pr_id." and hide=1 order by sort asc,id asc");
        $nm = $h->num_rows($sm);
        if($nm){
            $i = 0;            
            ...
            $sd = $h->query("select id,detail_vn,detail_en,week,day from lessons where lesson_id=$idmm and year=$yr and month=".$_SESSION['mnn']." order by id asc");
            $id = 0;
        ...
        $msm .= '<a class="add" rel="'.$pr_id.'">'._addnew.'</a>';
    echo $msm.';;;'.$msd.';;;'.$mww.';;;'.$mdd;
?>