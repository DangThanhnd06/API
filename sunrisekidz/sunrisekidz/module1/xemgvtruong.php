<?php
    ...
    $sci = $_POST['idt'];
    $s = $h->query("select tieude_vn,tieude_en from school_brand where id=$sci");
    $r = $h->fetch_array($s);
    $mss = $r["tieude_$lang"];
    ...
    if($n) {
                            while($rht = $h->fetch_array($sht)){
                                ...
                                $ms .= '<li><a class="tree" rel="'.$rht['id'].'" type="0"><img src="img/right_ht.png" class="htt" /></a><a class="xemgv" rel="'.$rht['id'].'">'._ht.' - '.$tengvht.'</a>';
                                ...
                                if($n){
                                    ...
                                    while($rgv = $h->fetch_array($sgv)){
                                        ...
                                        $ms .= '<li><a class="xemgv" rel="'.$rgv['id'].'" >'.$tengv.'</a></li>';
                                    }
                                    $ms .= '</ul>';
                                }
                                $ms .= '</li>';
