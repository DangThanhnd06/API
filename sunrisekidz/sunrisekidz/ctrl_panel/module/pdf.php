<?php	
	$act = $_REQUEST['act'];
    $actt = "act=pdf";
    $quanly = 'Quản lý PDF Catalogue';
	$them = "act=them_pdf";
	$update = "act=update_pdf";
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			@$result = $h->delete("h_pdf"," where id=".$id);
...
<?php
	...
	$hit = '<tr>
     ...
     if(isset($_REQUEST['tukhoa'])) { 
		$key = $_REQUEST['tukhoa'];
            $where = " where tieude_vn like '%$key%'";
	}
	else {
		$key = '';
		  $where = "";
	...
	if(isset($_POST['action']) && $_POST['action']=='Cập nhật') {
		$s = $h->query("select id from h_pdf $where");
		$nums = $h->num_rows($s);
		//echo $nums;
		$i=0;
		while($i<$nums){
			...
			$query = "update h_pdf set sapxep='$sapxep',hienthi='$hienthi' where id=".$id;
			mysql_query($query);
...
<?php
	if($key=="")
		$pageindex=taotrang("select count(id) from h_pdf $where","../ctrl_panel/?$actt&page=",20,$page);
	else
		$pageindex=taotrang("select count(id) from h_pdf $where","../ctrl_panel/?$actt&tukhoa='".$key."'&page=",20,$page);
	
	$sn = $h->query("select id from h_pdf $where");
	$n = $h->num_rows($sn);
?>
...
<p class="title"><?php echo $quanly?></p>
			...
			<tr>
				...
				<tr class="ko">
					<td class="smallfont ko">&nbsp;&nbsp;Nhập từ khóa tìm kiếm (tiêu đề):</td>
			...
			?>
              ...
              <?php echo $hit; } else echo $hit;
			  		if($key!="")
					{
						$result = $h->query("select id,sapxep,tieude_vn,hinhanh,hienthi from h_pdf $where order by id desc limit ".($p*20).",20");
			  ...
			  <tr>
				...
				<td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="42%" valign="middle"><a href="../ctrl_panel/?<?php echo $update?>&id=<?php echo $row['id']?>" title="Cập nhật"><?php echo $row['tieude_vn']; ?></a></td>
                <?php if($row['hinhanh'] != '') { ?>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="20%" valign="middle"><a href="../ctrl_panel/?<?php echo $update?>&id=<?php echo $row['id']?>" title="Cập nhật"><img src="../upload/pdf/<?php echo $row['hinhanh']; ?>" width="100" /></a></td>
                <?php } else { ?>
