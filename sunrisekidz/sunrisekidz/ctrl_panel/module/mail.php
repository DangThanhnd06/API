<?php	
	$act = $_REQUEST['act'];
	$quanly = "Quản lý mail nhận khuyến mãi";
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			@$result = $h->delete("h_mail"," where id=".$id);
...
<?php
	if(isset($_POST['action']) && $_POST['action']=='Xóa chọn') {
		...
		$hit = '<tr>
                ...
                <td align="center" nowrap class="title" width="85%"><b>Email</b></td>
...
<?php
		$pageindex=taotrang("select count(id) from h_mail","../ctrl_panel/?act=$act&page=",20,$page);
	
	$sn = $h->query("select id from h_mail");
	$n = $h->num_rows($sn);
?>
...
<p class="title"><?php echo $quanly?></p>
            ...
            <table class="trangquanli" width="772">
              ...
              <?php echo $hit;
			  		$result = $h->query("select id,hoten,sdt,yahoo,skype,sapxep,hienthi,email from h_mail order by id desc limit ".($p*20).",20" );
					$i=0;
			  ...
			  <tr>
				...
				<td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="15%" valign="middle"><input type="text" name="hoten[<?php echo $i?>]" value="<?php echo $row['hoten']?>" size="30" style="text-align:center;" /></td>
                ...
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="15%" valign="middle"><input type="text" name="yahoo[<?php echo $i?>]" value="<?php echo $row['yahoo']?>" size="20" style="text-align:center;" /></td>
