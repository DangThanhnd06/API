<?php	
    ...
    $quanly = 'Quản lý'.$ql;
	$them = "act=them_tintuc&tt_id=".$tt_id;
	$update = "act=update_tintuc&tt_id=".$tt_id;
	if(isset($_REQUEST['tukhoa'])) { 
		$key = $_REQUEST['tukhoa'];
            $where = " where tt_id=$tt_id and tieude like '%$key%'";
	}
	else {
		$key = '';
		  $where = " where tt_id=$tt_id";
	}
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			@$result = $h->delete("h_tintuc"," where id=".$id);
...
<?php
	...
	if(isset($_POST['action']) && $_POST['action']=='Cập nhật') {
		$s = $h->query("select id from h_tintuc $where");
		$nums = $h->num_rows($s);
		//echo $nums;
		$i=0;
		while($i<$nums){
			...
			$query = "update h_tintuc set sapxep='$sapxep',hienthi='$hienthi' where id=".$id;
			mysql_query($query);
...
<?php
	if($key=="")
		$pageindex=taotrang("select count(id) from h_tintuc $where","../ctrl_panel/?$actt&page=",20,$page);
	else
		$pageindex=taotrang("select count(id) from h_tintuc $where","../ctrl_panel/?$actt&tukhoa='".$key."'&page=",20,$page);
	
	$sn = $h->query("select id from h_tintuc $where");
	$n = $h->num_rows($sn);
?>
...
<p class="title"><?php echo $quanly?></p>
			...
			?>
              ...
              <?php echo $hit; } else echo $hit;
			  		if($key!="")
					{
						$result = $h->query("select id,sapxep,tieude,hinhanh,hienthi from h_tintuc $where order by id desc limit ".($p*20).",20");
			  ...
			  <tr>
                ...
                <?php if($row['hinhanh'] != '') { ?>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="20%" valign="middle"><a href="../ctrl_panel/?<?php echo $update?>&id=<?php echo $row['id']?>" title="Cập nhật"><img src="../upload/news/<?php echo $row['hinhanh']; ?>" width="100" /></a></td>
                <?php } else { ?>
