<?php	
	$act = $_REQUEST['act'];
    //$actt = "act=pdf";
    $quanly = 'Quản lý học sinh';
	$them = "act=them_hocsinh";
	$update = "act=update_hocsinh";
	...
	switch ($_GET['action'])
	{
		case 'del' :
			$id = $_GET['id'];
			@$result = $h->delete("h_hocsinh"," where id=".$id);
...
<?php
	...
	$hit = '<tr>
				...
				<td align="center" nowrap class="title" width="22%"><b>Họ tên</b></td>
				<td align="center" nowrap class="title" width="10%"><b>Năm sinh</b></td>
                <td align="center" nowrap class="title" width="21%"><b>Giáo viên phụ trách</b></td>
                <td align="center" nowrap class="title" width="13%"><b>Đổi user hoặc mật khẩu</b></td>
                <td align="center" nowrap class="title" width="10%"><b>Xem bảng điểm</b></td>
     ...
     if(isset($_POST['action']) && $_POST['action']=='Cập nhật') {
		$s = $h->query("select id from h_hocsinh");
		$nums = $h->num_rows($s);
		//echo $nums;
		$i=0;
		while($i<$nums){
			...
			$query = "update h_hocsinh set kichhoat='$hienthi' where id=".$id;
			mysql_query($query);
...
<?php
    if($key=="")
	   $pageindex=taotrang("select count(id) from h_hocsinh","../ctrl_panel/?act=hocsinh&page=",20,$page);
	else
        $pageindex=taotrang("select count(id) from h_hocsinh where hoten like '%$key%'","../ctrl_panel/?act=hocsinh&tukhoa=$key&page=",20,$page);
	$sn = $h->query("select id from h_hocsinh $where");
	$n = $h->num_rows($sn);
?>
...
<p class="title"><?php echo $quanly?></p>
			...
			<tr>
                ...
                <tr class="ko">
					<td class="smallfont ko">&nbsp;&nbsp;Nhập từ khóa tìm kiếm (họ tên HS):</td>
			...
			?>
              ...
              <?php echo $hit; } else echo $hit;
			  		if($key!="")
					{
						$result = $h->query("select id,hoten,id_gv,ngaysinh,kichhoat from h_hocsinh $where order by id desc limit ".($p*20).",20");
			  ...
			  <tr>
				...
				<td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="22%" valign="middle"><a href="../ctrl_panel/?<?php echo $update?>&id=<?php echo $row['id']?>" title="Cập nhật"><?php echo $row['hoten']; ?></a></td>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="10%" valign="middle"><a href="../ctrl_panel/?<?php echo $update?>&id=<?php echo $row['id']?>" title="Cập nhật"><?php echo $row['ngaysinh']; ?></a></td>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="21%" valign="middle"><?php echo $gv; ?></td>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="13%" valign="middle"><a href="../ctrl_panel/?act=doiumhs&id=<?php echo $row['id']?>" title="Đổi tên đăng nhập hoặc mật khẩu phụ huynh này">Thay đổi</a></td>
                <td align="left" bgcolor="<?php echo $color?>" class="smallfont" width="10%" valign="middle"><a href="../ctrl_panel/?act=bangdiem&id=<?php echo $row['id']?>" title="Xem bảng điểm của học sinh <?=$row['hocsinh']?>">Xem</a></td>
				...
				<td width="20" align="center" bgcolor="<?php echo $color; ?>" class="smallfont" valign="middle">
				<a onClick="return confirm('Bạn chắc chắn muốn xóa?');" href="../ctrl_panel/?act=hocsinh&action=del&id=<?php echo $row['id']; ?>" title="Xóa học sinh này"><img src="icons/icon_delete.gif" width="16" height="16" border="0" /></a></td>
			  </tr>
