<?php	
	$act = $_REQUEST['act'];
	$quanly = 'Quản lý danh mục sản phẩm';
	
	$them = 'them_loaisp';
	$update = 'update_loaisp';
	$page = 0;
	...
	if(isset($_POST['action']) && $_POST['action']=='Cập nhật') {
		...
		while($i<$nums){
			...
			$query = "update h_sanpham set sapxep='$sapxep',hienthi='$hienthi' where id=".$id;
...
<?php
	$pageindex=taotrang("select count(id) from h_sanpham where sp_id=0","../ctrl_panel/?act=nhac&page=",20,$page);
...
<div class="tmcn">
            ...
            <table width="100%" class="trangquanli">
			  ...
			  <?php 
			  		$result = $h->query("select * from h_sanpham where sp_id=0 order by id desc limit ".($p*20).",20" );
					$i=0;
			  ...
			  <tr>
                ...
                <td align="center" bgcolor="<?php echo $color?>" class="smallfont" width="70%" valign="middle"><?php echo $row['tieude']; ?></td>
				...
				<td width="20" align="center" bgcolor="<?php echo $color; ?>" class="smallfont" valign="middle">
				<a onClick="return confirm('Bạn có chắc chắn muốn xóa ?');" href="../ctrl_panel/?act=<?php echo $act?>&action=del&id=<?php echo $row['id']; ?>" title="Xóa"><img src="icons/icon_delete.gif" width="16" height="16" border="0" /></a></td>
			  </tr>
			  <?php
						} 
			  ?>
