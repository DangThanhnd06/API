<div id="content">
                <div class="leftcr">
                    ...
                    <div class="scrollbar-macosx">
                        ...
                        <?php
                            if($_SESSION['quyenper'] == 2)
                                $sc = $h->query("select id,fname,mname from children where hide=1 and school_id=".$_SESSION['schd']." order by fname asc");
                            else
                            ...
                            while($rc = $h->fetch_array($sc)){
                                ...
                                echo '<li '.$cl.'><a class="xemhs" rel="'.$rc['id'].'"><span class="numb">'.$i.'</span>'.$fullname.'</a><a class="delhs" rel="'.$rc['id'].'">&nbsp;</a></li>';
                            }
                        ?>  
                        </ul>
                    </div>
                    <div class="addnew">
                        <a class="addcr"><?=_addnew?></a>
                    </div>
                </div>
                <div class="rightcr">
        ...
        <!-- change father's password -->
        <div id="popup22" class="ghpopup">
            <p class="kidz"><img src="img/bg_baby.png" alt="Baby" width="100" /></p>
            <h2 class="tieudepop"><?=_cpoffathers?></h2>
            <div class="form scrollbar-macosx">
                <form method="post" action="module/children/changepassfather.php" id="formcpfather" enctype="multipart/form-data">
                    <input type="hidden" id="idff" name="idff" value="" />
                    <div>
                        <label for="fname"><?=_newpassword?></label>
                        <input type="password" name="fnp" id="fnp" class="in" />
                    </div>
                    <div>
                        <label for="mname"><?=_renewpassword?></label>
                        <input type="password" name="frnp" id="frnp" class="in" />
                    </div>
                    <div>
                        <input type="submit" class="gui" id="changepassfather" value="<?=_changepass?>" />
                    </div>
                </form>
            </div>
        </div>
        <!-- change mother's password -->
        <div id="popup23" class="ghpopup">
            <p class="kidz"><img src="img/bg_baby.png" alt="Baby" width="100" /></p>
            <h2 class="tieudepop"><?=_cpofmothers?></h2>
            <div class="form scrollbar-macosx">
                <form method="post" action="module/children/changepassmother.php" id="formcpmother" enctype="multipart/form-data">
                    <input type="hidden" id="idmm" name="idmm" value="" />
                    <div>
                        <label for="fname"><?=_newpassword?></label>
                        <input type="password" name="mnp" id="mnp" class="in" />
                    </div>
                    <div>
                        <label for="mname"><?=_renewpassword?></label>
                        <input type="password" name="mrnp" id="mrnp" class="in" />
                    </div>
                    <div>
                        <input type="submit" class="gui" id="changepassmother" value="<?=_changepass?>" />
                    </div>
                </form>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function(){
            ...
            $('input.se').live('keydown',function(e) {
           	    if (e.keyCode == 13) {
              		moduleSearch();
               	}
            });
            ...
            /*
            $('input.se').live("keyup",function(){
                var keyw = $(this).val();
                $.post("module/search_child.php", {keyw: keyw}, function(data){
                    $("ul.headteacher").fadeIn('slow').html(data);        
                });
            });
            */
            $('.delhs').live("click",function(){
                
               var id = $(this).attr('rel');
               var ids = $('#idcrrr').val();
               $('.thongbao').html('<?=_condelchild?><p class="p"><a class="ok" rel="'+id+'" style="margin:0 10px 0 0;"><?=_ok?></a><a class="cancel"><?=_cancel?></a>');
               $('#popup16').bPopup();
               $('.ok').live("click",function(){
                    var idd = $(this).attr('rel');
                    var chame = $('a.delhs[rel="'+idd+'"]').parent();
                    $.post("module/children/del_child.php",{id: idd}, function(data){
                        if(data == 1){
                            bclose16();
                            $('p.thongbao').html('<?=_xoattc?>');
                            $('#popup5').bPopup({
                                autoClose: 1500
                            });
                            if(idd == ids) 
            ...
            // change father's' password
            jQuery('.changepf').live("click",function(){
            ...
            // change mother's' password
            jQuery('.changepm').live("click",function(){
            ...
            // #changepassfather click
            jQuery('#changepassfather').live('click',function(){
               if($('#fnp').val()==''){
               ...
               if($('#frnp').val()==''){
                $('p.thongbao').html("<?=_norenewpassword?>");
                $('#popup5').bPopup({
                   autoClose: 1500 
                });
                $('#frnp').focus();
               ...
               if($('#fnp').val() != $('#frnp').val()){
                $('p.thongbao').html("<?=_passwordnomatch?>");
                $('#popup5').bPopup({
                   autoClose: 1500 
                });
                $('#frnp').focus();
               ...
               // formcpfather
               $('#formcpfather').ajaxForm({
                    beforeSend: function() {
                        $('#changepassfather').attr("disabled",true);
                        $('#loading').show();
                    },
                    uploadProgress: function(event, position, total, percentComplete) {
                    ...
                    success: function() {
                   	...
                   	complete: function(xhr) {
                	    $('#loading').hide();
                        $('#changepassfather').removeAttr('disabled');
            ...
            // #changepassmother click
            jQuery('#changepassmother').live('click',function(){
               if($('#mnp').val()==''){
                $('p.thongbao').html("<?=_nonewpass?>");
                $('#popup5').bPopup({
                   autoClose: 1500 
               ...
               // formcpfather
               $('#formcpmother').ajaxForm({
                    beforeSend: function() {
                        $('#changepassmother').attr("disabled",true);
                        $('#loading').show();
                    },
                    uploadProgress: function(event, position, total, percentComplete) {
                    ...
                    success: function() {
                   	...
                   	complete: function(xhr) {
                	    $('#loading').hide();
                        $('#changepassmother').removeAttr('disabled');
               	        bclose23();
           				$('p.thongbao').html('<?=_changepsucc?>');
                        $('#popup5').bPopup({
                            autoClose: 1500
            ...
            $('#cse').live('change',function(){
                var csee = $(this).val();
                $.post("module/children/show_teacher.php",{csee: csee}, function(data){
