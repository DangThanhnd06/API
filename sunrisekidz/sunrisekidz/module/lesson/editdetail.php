<?php
    ...
    $data['mota_vn'] = str_replace("'","\'",trim($_POST['mota_vn']));
    $data['mota_en'] = str_replace("'","\'",trim($_POST['mota_en']));
    /*
    ...
    */
    ...
    $rl = $h->fetch_array($sl);
    $l_id = $rl['lesson_id'];
    $ss = $h->query("select * from lessons where lesson_id=$l_id order by week asc,day asc,id asc");
    $i = 0;
    $ms = "";
    while($r = $h->fetch_array($ss)) {
        ...
        $dell = $r["detail_$lang"];
        if($r["mota_$lang"] != '') $mota = '<br />'._desdetail.': '.$r["mota_$lang"];
        else $mota = '';
        $ms .= '<p class="cten'.$cld.$cla.'"><a class="dell" rel="'.$idd.'">'.$dell.$mota.'</a><a class="editde" rel="'.$idd.'">&nbsp;</a><a class="delde" rel="'.$idd.'">&nbsp;</a></p>';
        $i++;
    }
