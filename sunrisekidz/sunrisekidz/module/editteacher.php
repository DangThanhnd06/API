<?php
    ...
    $data['namvaotruong'] = trim($_POST['namvaotruonge']);
    ...
    if($_FILES['img2']['name']!='') {
        if($_FILES["img2"]["type"]=='image/jpeg' || $_FILES["img2"]["type"]=='image/gif' || $_FILES["img2"]["type"]=='image/png' || $_FILES['img2']['type']=='image/jpg') {
				...
				move_uploaded_file($_FILES['img2']['tmp_name'],$path."/".$id.'-'.chuoianh($_FILES['img2']['name']));
        }
        $data['img'] = $id.'-'.chuoianh($_FILES['img2']['name']);
    } else $data['img'] = $imgg;
    ...
    if($ntc) echo '3';
        ...
        if($ns) echo '2';
            ...
            $msr .= '   <p><strong>'._namvaotruong.': </strong>'.$rg['namvaotruong'].'</p>';
            $msr .= '</div>';
            $msr .= '<p class="clr"></p>';
