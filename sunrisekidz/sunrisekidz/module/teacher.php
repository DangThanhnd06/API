            <div id="content">
                ...
                <?php
                    if($_SESSION['quyenper'] == '2'){
                        $ssb1 = $h->query("select id,tieude from school_brand where id=".$_SESSION['schd']." order by id asc");
                    } else {
                        $ssb1 = $h->query("select id,tieude from school_brand order by id asc");    
                    }
                    
                    $rsb1 = $h->fetch_array($ssb1);
                    if($lang == 'vn') $tentt1 = $rsb1['tieude'];
                    else $tentt1 = change($rsb1['tieude']);
                    $fids = $rsb1['id'];
                ?>
                    <div class="school"><p style="display: block;" id="showsch"><?=_school?> <span id="tentruong"><?=$tentt1?></span></p>
                        <!--<div class="cschool"><p class="csc">&nbsp;</p></div>-->
                        <?php if($_SESSION['quyenper'] == '1'){ ?>
                        <div class="schoolhide">
                        <?php
                            $ssb = $h->query("select id,tieude from school_brand order by id asc");
                            $nsb = 1;
                            while($rsb = $h->fetch_array($ssb)){
                                if($lang == 'vn')
                                    $truongbr = $rsb["tieude"];
                                else
                                    $truongbr = change($rsb["tieude"]);
                                $id = $rsb['id'];
                                if($nsb == 1) $_SESSION['idcss'] = $rsb['id'];
                    ...
                    <div class="scrollbar-macosx" id="scss">
                        ...
                        <?php
                            ...
                            $sht = $h->query("select id,fname,mname,lname from teacher where level=1 and school_id=$fids and hide=1 order by sort asc,id asc");
                            while($rht = $h->fetch_array($sht)){
                                ...
                                echo '<li><a class="tree" rel="'.$rht['id'].'" type="0"><img src="img/right_ht.png" class="htt" /></a><a class="xemgv" rel="'.$rht['id'].'">'._ht.' - '.$tengvht.'</a><a class="delgv" rel="'.$rht['id'].'">&nbsp;</a>';
                                ...
                                if($n){
                                    ...
                                    while($rgv = $h->fetch_array($sgv)){
                                        ...
                                        echo '<li><a class="xemgv" rel="'.$rgv['id'].'" >'.$tengv.'</a><a class="delgv" rel="'.$rgv['id'].'">&nbsp;</a></li>';
                                    }
                                    echo '</ul>';
                                }
                                echo '</li>';
                    ...
                    <div class="addnew">
                        <a class="addteacher"><a href="javascript:void(0)" class="addtc"><?=_addnew?></a></a>
                    </div>
                </div>
                <div class="righttc">
        ...
        <div id="popup" class="ghpopup">
            ...
            <div class="form scrollbar-macosx">
                <form method="post" action="module/addteacher.php" id="formadtc" enctype="multipart/form-data">
                    ...
                    <?php } else { ?>
                        <input type="hidden" name="cs" id="css" value="<?=$_SESSION['schd']?>" />
                        <?php } ?>
                    <!--
                    <div>
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function(){
            $('select#sce').live("change",function(){
                var sci = $('#sce').val();
                var cl = $('#cl').val();
                if(sci==0) {
                    $('#lisths').html('');
            ...
            jQuery('.addtc').click(function(){
                var tentr = $('#tentruong').text();
                $.post("module/load_school.php", {tt: tentr}, function(data){
                    $('#cs').html(data);
                });
                $('#popup').bPopup({
            	   speed: 650,
            ...
            $('.delgv').live("click",function(){
               var id = $(this).attr('rel');
               $('.thongbao').html('<?=_condelteacher?><p class="p"><a class="ok" rel="'+id+'" style="margin:0 10px 0 0;"><?=_ok?></a><a class="cancel"><?=_cancel?></a>');
               $('#popup16').bPopup();
               $('.ok').live("click",function(){
                    var idd = $(this).attr('rel');
                    var chame = $('a.delgv[rel="'+idd+'"]').parent();
                    $.post("module/del_teacher.php",{id: idd}, function(data){
                        if(data == 1){
                            bclose16();
                            ...
                            bclose16();
                            if(data == 2){
                                $('p.thongbao').html('<?=_stillhead?>');
                                $('#popup5').bPopup({
                                    autoClose: 3500
                                });
                            } else {
                                if(data == 3){
                                    $('p.thongbao').html('<?=_stillcl?>');
                                    $('#popup5').bPopup({
                                        autoClose: 3500
                                    });
                                }
            ...
            $('select#level').live("change",function(){
                ...
                if($('select#level option:selected').val()==2) {
                    $.post("module/show_headadd.php",{sce: sce}, function(data){
                            $('#reportto').fadeIn('slow').html(data);
					});
                }
                if($('select#level option:selected').val()==1) {
                    $('#reportto').html('<option value="0"><?=_choseteacher?></option>');
                }
            }).change();
            $('select#levele').live("change",function(){
                var sce = $('#cse').val();
                if($('select#levele option:selected').val()==2) {
                    $.post("module/show_headadd.php",{sce: sce}, function(data){
                            $('#reporttoe').fadeIn('slow').html(data);
					});
                }
                if($('select#levele option:selected').val()==1) {
                    $('#reporttoe').html('<option value="0"><?=_choseteacher?></option>');
                }
            }).change();
            ...
            $('#guiedit').live("click",function(){
                ...
                var nvt = $('#namvaotruonge').val();
                if(nvt == ""){
                    $('p.thongbao').html('<?=_nonamvaotruong?>');
                    $('#popup5').bPopup({
                        autoClose: 1500
                    });
                    $('#namvaotruonge').focus();
                    ...
                    if(isNaN(nvt) || nvt.length < 4 || nvt.length > 4){
                        $('p.thongbao').html('<?=_invalidnamvaotruong?>');
                        $('#popup5').bPopup({
                            autoClose: 1500
                        });
                        $('#namvaotruonge').focus();
